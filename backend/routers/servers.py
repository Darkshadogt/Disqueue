...
def _raise_for_status_with_rate_limit(res: httpx.Response) -> None:
    """
    Discord's 429 response has a JSON body with retry_after; surface that
    to the client as a 503 instead of letting raise_for_status() turn a
    transient rate limit into an opaque, unhelpful 500.
    """
...
async def get_my_linked_servers(user=Depends(get_current_user)):
    """
    Returns every server the user shares with the bot
    """
    ...
    async with httpx.AsyncClient() as client:
        # Guilds the bot itself is currently a member of.
        ...
        # Guilds this user belongs to, with their permission flags for each
